...
| vol5 | **完走(2026-07-31)** | 生成モデル編(2026-07-29 開始)。vol4 パターンを踏襲し、前半(ステップ 1〜5)は素の Rust、ステップ 6(NN/VAE 境界)入り口でクレート昇格を判断。**前半完走(2026-07-29)**: ステップ 1〜3 正規分布・最尤推定・多変量正規(det/inv は BLAS に頼らず自作)/ ステップ 4 GMM(Cholesky も自作し `np.random.multivariate_normal` の中身まで写経、標本共分散 1 万点の統計 assert)/ ステップ 5 EM(**Old Faithful 実データの収束先が文献の正典値 (2.04, 54.5)・(4.29, 80.0) と一致**、対数尤度の単調性定理を誤差付き assert に翻訳。**f32 アンダーフロー事件**: 本の f64 前提の初期値が f32 では負担率崩壊 → スケール調整で回避)/ ステップ 6 読章+**PyTorch 学習パリティ予行**(初期重みを npz で輸入し vol3 で同一学習 — iter 0 の loss ビット一致、10⁴ iter で loss drift 6.1e-6・重み max\|Δ\| 2.35e-3 → 「三段 tol」ドクトリン確立)/ ステップ 7 **VAE**(vol3 の既存部品のみで合成 — 新規 Function ゼロ。**パリティ儀式が KL 係数の流儀混在を初検出**(単体テストでは原理的に不可捕)。10 iter パリティ diff 4.6e-5 ≈ f32 ε、30 epoch の統計収束 38.8194 vs PyTorch 38.8193、z〜N(0,I) から 64 枚生成の PGM 出力)/ ステップ 8〜9 **拡散モデル**(理論は「逐次ノイズ付加 vs 閉形式」の統計 assert に翻訳。vol3 に Concat・bilinear Upsample・BatchNorm2d を増設 — 随伴ペア設計・**実装前に罠を golden 化する事前登録制**。UNet 学習パリティ: **iter 0 の loss 7 桁一致**、ドリフトは重み/BN 統計の 2 クラス tol、縮小構成の統計 band 0.0533 vs 0.0596)/ ステップ 10 **条件付き拡散+Classifier-Free Guidance**(label 埋め込みは one-hot × Linear の合成で新規部品ゼロ、CFG は 2 回 forward の γ ブレンド — diffusion.rs 無改造。golden は drop_flags 手固定で両分岐を強制、iter 0 の loss 7 桁一致・CFG サンプリング部分軌道 <1e-4)/ **卒業制作**: 本構成(60k 枚・batch 128・10 epoch)を一晩 8.1 時間で学習、γ=3.0・T=1000 の CFG 生成で **10 ラベル × 4 行の 40 枚が全て指定クラス通り** — 本の最終図を自作エンジンで再現 |
| vol6 | 未着手    | —                                                                                      |
...
2. **vol4(強化学習)**: 前半(バンディット〜TD 法)は素の Rust で完走(2026-07-28)。
   ...
   **vol5(生成モデル)**は SBI 研究に直結する巻で、フレームワーク成長の主要ドライバ。
   後半(ステップ 6〜10、PyTorch 写経区間)も vol3 への path 依存で続行する(2026-07-29 決定)。
3. **本番フレームワークの新規構築(vol5 完走後・別リポジトリ)**: 昇格・移設ではなく、
   vol3 を「自分で書いた仕様書 = golden 参照」として独立ソフトウェアを新規に建てる
   (書き手は Claude — 本番ツール化モード)。取り込みは git submodule ではなく Cargo の
   git/path 依存。写経で得た失敗と要求は [docs/framework-v1-notes.md](docs/framework-v1-notes.md)
   に台帳として蓄積し、これを設計仕様の初稿とする。
4. **PyTorch パリティ検証**を「本番昇格の儀式」として整備(vol5 後半の写経が実地訓練を兼ねる)。
...
├── vol5/            # 5巻目(生成モデル編)のクレート — 前半は素の Rust、6〜は vol3 に path 依存
│   ├── src/
│   │   ├── gaussian.rs   # ステップ1〜4 正規分布・GMM(det/inv/Cholesky 自作・サンプリング)
│   │   ├── em.rs         # ステップ5 EM アルゴリズム(GaussianMixtureModel)
│   │   ├── vae.rs        # ステップ7 VAE(reparameterize・KL・Encoder/Decoder)
│   │   ├── diffusion.rs  # ステップ8〜9 拡散スケジューラ(add_noise/denoise、ᾱ_0≡1)
│   │   ├── unet.rs       # ステップ9〜10 UNet(ConvBlock・pos_encoding・ラベル埋め込み)
│   │   └── utils.rs      # approx_eq・random_normal_array(RNG 注入)・load_txt
│   ├── tests/            # ステップ単位の実験+assert(step01_03.rs〜)
│   ├── tools/            # PyTorch golden 生成スクリプト(step06/07/09/10 パリティ用)
│   └── examples/
│       └── step07_generate.rs # 7.4.4 VAE 生成画像の PGM グリッド出力
...
| ステップ 6 ニューラルネットワーク | (読章 — 全節 vol3 で自作済み。代わりに **PyTorch 学習パリティ予行**: `tools/gen_step06_golden.py` + `tests/step06.rs`) |
| ステップ 7 変分オートエンコーダ(VAE) | `vol5/src/vae.rs`(実験: `tests/step07.rs` — 三段 tol パリティ、golden: `tools/gen_step07_golden.py`。生成: `examples/step07_generate.rs`) |
| ステップ 8 拡散モデルの理論   | (読章+理論検算 — `vol5/src/diffusion.rs` のスケジューラに「逐次 vs 閉形式」の統計 assert として埋め込み) |
| ステップ 9 拡散モデルの実装   | `vol5/src/diffusion.rs`・`unet.rs`(vol3 拡張: Concat・UpsampleBilinear2x・BatchNorm2d。実験: `tests/step09.rs` — 三段 tol パリティ、golden: `tools/gen_step09_golden.py`) |
| ステップ 10 拡散モデルの応用  | `vol5/src/unet.rs`(label_emb 追加。実験: `tests/step10.rs` — CFG パリティ、golden: `tools/gen_step10_golden.py`。卒業制作: `examples/step10_train.rs`(毎 epoch save_weights)+`step10_generate.rs`(CLI で重み・γ 差替可)) |
...
第5巻(vol5)の VAE の 30 epoch 実データ学習(帯域検証)と生成例。golden は
`vol5/tools/gen_step06_golden.py` / `gen_step07_golden.py`(要 PyTorch venv)で再生成できる:

```sh
cargo test -p vol5 --release --test step07 -- --ignored --nocapture  # 7.4 VAE 30 epoch 帯域検証(約80秒)
cargo run -p vol5 --example step07_generate --release                # 学習+z〜N(0,I) 64枚生成(vol5/output/vae_generated.pgm)
cargo test -p vol5 --release --test step09 -- --ignored --nocapture  # 9章 拡散モデル tier3 統計パリティ(約4分)
```

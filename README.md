...
| vol5 | **進行中(前半完走)** | 生成モデル編(2026-07-29 開始)。vol4 パターンを踏襲し、前半(ステップ 1〜5)は素の Rust、ステップ 6(NN/VAE 境界)入り口でクレート昇格を判断。**前半完走(2026-07-29)**: ステップ 1〜3 正規分布・最尤推定・多変量正規(det/inv は BLAS に頼らず自作)/ ステップ 4 GMM(Cholesky も自作し `np.random.multivariate_normal` の中身まで写経、標本共分散 1 万点の統計 assert)/ ステップ 5 EM(**Old Faithful 実データの収束先が文献の正典値 (2.04, 54.5)・(4.29, 80.0) と一致**、対数尤度の単調性定理を誤差付き assert に翻訳。**f32 アンダーフロー事件**: 本の f64 前提の初期値が f32 では負担率崩壊 → スケール調整で回避)/ ステップ 6 読章+**PyTorch 学習パリティ予行**(初期重みを npz で輸入し vol3 で同一学習 — iter 0 の loss ビット一致、10⁴ iter で loss drift 6.1e-6・重み max\|Δ\| 2.35e-3 → 「三段 tol」ドクトリン確立)/ ステップ 7 **VAE**(vol3 の既存部品のみで合成 — 新規 Function ゼロ。**パリティ儀式が KL 係数の流儀混在を初検出**(単体テストでは原理的に不可捕)。10 iter パリティ diff 4.6e-5 ≈ f32 ε、30 epoch の統計収束 38.8194 vs PyTorch 38.8193、z〜N(0,I) から 64 枚生成の PGM 出力)/ ステップ 8〜9 **拡散モデル**(理論は「逐次ノイズ付加 vs 閉形式」の統計 assert に翻訳。vol3 に Concat・bilinear Upsample・BatchNorm2d を増設 — 随伴ペア設計・**実装前に罠を golden 化する事前登録制**。UNet 学習パリティ: **iter 0 の loss 7 桁一致**、ドリフトは重み/BN 統計の 2 クラス tol、縮小構成の統計 band 0.0533 vs 0.0596) |
| vol6 | 未着手    | —                                                                                      |
...
│   │   ├── unet.rs       # ステップ9 UNet(ConvBlock・時刻埋め込み pos_encoding)
│   │   └── utils.rs      # approx_eq・random_normal_array(RNG 注入)・load_txt
│   ├── tests/            # ステップ単位の実験+assert(step01_03.rs〜)
│   ├── tools/            # PyTorch golden 生成スクリプト(step06/step07 パリティ用)
│   └── examples/

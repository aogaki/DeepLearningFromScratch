...
mod tests {
    ...
    #[test]
    #[ignore] // cargo test --release test_train_mnist_deep_gpu -- --ignored --nocapture
    fn test_train_mnist_deep_gpu() {
        ...
        println!("--- Training GpuDeepConvNet (SGD) ---");
        println!(
            "Max Epochs: {}, Batch Size: {}, Total Iters: {}",
            max_epochs, batch_size, iters_num
        );
        println!(
            "(Run with: cargo test --release test_train_mnist_deep_gpu -- --ignored --nocapture)"
        );

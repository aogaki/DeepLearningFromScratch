...
| vol3 | **進行中** | 第1ステージ(ステップ1〜10)完了: `Variable`(`Rc<RefCell>` の薄いハンドル — Python の共有参照を Rust で明示)/ Function を `Forward`・`Function`・`Creator` の 3 trait に分割し、`call(self)` が関数を `Node<F>` としてグラフへ移す(「入力未設定」状態が型で排除される設計)/ 数値微分(f32 では eps ≈ ∛ε ≈ 5e-3 と導出、本の 1e-4 は float64 用)/ 自動逆伝播(再帰ではなくループ)/ 勾配チェック(backprop vs 数値微分) |
| vol4 | 未着手    | —                                                                                      |
| vol5 | 未着手    | —                                                                                      |
| vol6 | 未着手    | —                                                                                      |
...
│   └── src/
│       └── core.rs  # ステップ1〜10: Variable・Function(trait 3分割)・数値微分・自動逆伝播・勾配チェック
...
| 本のステップ                   | ファイル                          |
| ------------------------------ | --------------------------------- |
| 第1ステージ(1〜10)Variable・Function・数値微分・自動逆伝播・勾配チェック | `vol3/src/core.rs` |
